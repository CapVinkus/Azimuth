package it.gepo.engine.local.writer;
...
public class CustomItemWriter<T> extends AbstractItemStreamItemWriter<List<T>>
implements ResourceAwareItemWriterItemStream<List<T>>, InitializingBean {
...
this.setExecutionContextName(ClassUtils.getShortName(CustomItemWriter.class));
}
...
* line separator is determined by the underlying operating system. If the input
* is not a String and a converter is available the converter will be applied
* and then this method recursively called with the result. If the input is an
* array or collection each value will be written to a separate line
* (recursively calling this method for each value). If no converter is supplied
* the input object's toString method will be used.<br>
* 
...
*/
@Override
public void write(List<? extends List<T>> lista) throws Exception {

for (List<T> items : lista) {
	...
	int lineCount = 0;
	for (T item2 : items) {
		lines.append(lineAggregator.aggregate(item2) + lineSeparator);
		lineCount++;
	}
	try {
